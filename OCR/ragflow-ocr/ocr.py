...
class TextRecognizer(object):
    def __init__(self, model_dir):
        self.rec_image_shape = [int(v) for v in "3, 48, 320".split(",")] # C, H , W
        self.rec_batch_num = 16 #batch
        postprocess_params = {
            'name': 'CTCLabelDecode',
    ...
    def resize_norm_img(self, img, max_wh_ratio): # max_wh_ratio 计算缩放后的宽度
        imgC, imgH, imgW = self.rec_image_shape

        assert imgC == img.shape[2]
        imgW = int((imgH * max_wh_ratio))
        w = self.input_tensor.shape[3:][0]#获取输入张量的宽度 w
        if isinstance(w, str):
            pass
